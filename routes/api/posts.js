...
router.delete('/comment/:post_id/:comment_id', authMiddleware, async (req, res) => {
  try {
    ...
    const index = post.comments.findIndex(comment => comment.user.toString() === req.user.id)
    post.comments.splice(index, 1)
    await post.save()

    res.json(post.comments)
  } catch (err) {
    console.error('Error DELETE /posts/comment/:post_id/:comment_id request, ', err.message)
    res.status(500).send('Error DELETE /posts/comment/:post_id/:comment_id request')
  }
})
